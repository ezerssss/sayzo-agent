...
- **`sayzo_agent/gui/hud/window.py::HudWindow`** — runs in the subprocess. Frameless, transparent, top-right of primary monitor, 420×640 canvas. Applies platform tweaks to prevent focus theft: macOS `NSStatusWindowLevel` + collection behavior `CanJoinAllSpaces | FullScreenAuxiliary | Transient | IgnoresCycle`; Windows `WS_EX_NOACTIVATE | WS_EX_TOOLWINDOW`. Reads stdin commands, forwards them via `window.evaluate_js("window.hudBridge.dispatch(...)")` into the React app.
- **React HUD app** at `gui/webui/src/HudApp.tsx` + `gui/webui/src/hud/*` — state machine over `pill / dot / card / toast / actionable` overlays, click-through on empty regions via `pointer-events: none`, FIFO queue for consent cards, max-3 visible toasts.
