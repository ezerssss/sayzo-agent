...
- `notify(title, body)` — fire-and-forget toast (capture saved, post-arm guidance, stream-open error, welcome). On macOS this routes through `UNUserNotificationCenter` via `desktop-notifier`; on Windows it's a WinRT toast.
- `ask_consent(title, body, yes_label, no_label, timeout_secs, default_on_timeout) -> "yes" | "no" | "timeout"` — used by the ArmController for all consent flows (whitelist, hotkey start/stop confirmation, end-of-meeting confirmation, check-in, meeting-ended). On **macOS** this is an `osascript display dialog` modal (via `sayzo_agent/consent_modal.py`) — chosen deliberately because banner-style toasts get silently dropped to Notification Center under Focus mode or "Banner Style: None", and consent decisions must be unmissable. On **Windows** it's a WinRT toast with two action buttons (reliable enough that a modal isn't needed).

Toggle all non-consent toasts with `SAYZO_NOTIFICATIONS_ENABLED=0`. Consent + end-of-meeting prompts are always on (they're how the user decides what gets captured). AUMID (`"Sayzo"`, set in `installer/windows/sayzo-agent.nsi` and passed to `DesktopNotifier(app_name="Sayzo")`) is required on Windows 10 for WinRT toasts + buttons to render. macOS needs `CFBundleIdentifier = com.sayzo.agent` (set in `sayzo-agent.spec`) plus a Developer-ID signature + Apple notarization (applied in CI — see `.github/workflows/build.yml`) for `UNUserNotificationCenter` to actually display toasts. Source-run dev builds are unsigned and fall back to `NSUserNotification` via `notify_mac_unsigned.py` (deprecated API but still functional through macOS 15) — that path is selected automatically by `notify.make_notifier` when `is_signed_bundle()` returns False.
