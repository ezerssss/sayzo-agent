...
- **Auto-update only re-opens Settings for user-initiated updates.** A silent boot-time staged-apply (`apply_staged_if_newer(where="boot")`) now shows **only** the "Sayzo updated" toast — it does NOT pop Settings. A user-initiated apply (Settings → Install update / tray / HUD install — all route through `where="quit"`) writes the `open_settings_after_update.flag` marker, which the relaunched agent consumes in `service()` to re-open Settings on About. The marker is the only signal that survives the relaunch to distinguish the two paths (Windows passes `--open-settings` for both; macOS's `open --args service` trips `looks_user_launched()` for both), so the `service()` open-settings decision branches `user_update` → About, `post_upgrade` → suppress, else the prior `--open-settings` / `looks_user_launched()` behavior. **`where != "boot"` ⟺ user-initiated** — don't add an automatic caller with a non-`boot` where or it'd pop Settings on a silent update.
