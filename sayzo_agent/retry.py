...
# Set on dropped-stub records (cheap-gate fail). These are local-only
# audit entries with no audio to upload, so the sweep must treat them as
# terminal and never warn about them. Distinguished from FAILED_PERMANENT
# so the UI / future analytics can tell the difference between "we tried
# to upload and gave up" and "we never intended to upload this in the
# first place".
...
def discarded_locally_state() -> dict:
    """Terminal upload state for dropped-stub records (cheap-gate fail).
    The sweep skips these because ``is_terminal`` is True; no warnings,
    no retries.
    ``last_error_kind = "discarded_locally"`` is a load-bearing breadcrumb
    for any future "why was this skipped?" UI.
    """
